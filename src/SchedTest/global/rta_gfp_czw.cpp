...
RTA_GFP_CZW::RTA_GFP_CZW()
    : GlobalSched(false, RTA, FIX_PRIORITY, NONE, "", "CZW") {}

RTA_GFP_CZW::RTA_GFP_CZW(TaskSet tasks, ProcessorSet processors,
...
ulong RTA_GFP_CZW::workload(const Task& task, ulong interval) {
...
ulong RTA_GFP_CZW::interference(const Task& tk, const Task& ti,
                                ulong interval) {
  return min(interval - tk.get_wcet() + 1,
             max((ulong)0, workload(ti, interval)));
}

ulong RTA_GFP_CZW::response_time(const Task& ti) {
