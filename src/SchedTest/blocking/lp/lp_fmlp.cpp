...
//cout<<"original response time:"<<response_time<<endl;
			ulong temp = fmlp_get_response_time(*ti, tasks, processors, resources);
...
ulong fmlp_get_response_time(Task& ti, TaskSet& tasks, ProcessorSet& processors, ResourceSet& resources)
{
	...
	lp_fmlp_objective(ti, tasks, processors, response_bound, vars, obj);

	response_bound.set_objective(obj);

	lp_fmlp_add_constraints(ti, tasks, processors, resources, response_bound, vars);
...
////////////////////Expressions////////////////////
void lp_fmlp_directed_blocking(Task& ti, Task& tx, TaskSet& tasks, FMLPMapper& vars, LinearExpression *exp, double coef)
{
//cout<<"coef:"<<coef<<endl;
	uint x = tx.get_id();
//cout<<"task:"<<x<<endl
...
void lp_fmlp_indirected_blocking(Task& ti, Task& tx, TaskSet& tasks, FMLPMapper& vars, LinearExpression *exp, double coef)
{
//cout<<"coef:"<<coef<<endl;
	uint x = tx.get_id();
	foreach(tx.get_requests(), request)
...
void lp_fmlp_preemption_blocking(Task& ti, Task& tx, TaskSet& tasks, FMLPMapper& vars, LinearExpression *exp, double coef)
{
//cout<<"coef:"<<coef<<endl;
	uint x = tx.get_id();
	foreach(tx.get_requests(), request)
...
void lp_fmlp_OD(Task& ti, TaskSet& tasks, ProcessorSet& processors, FMLPMapper& vars, LinearExpression *exp, double coef)
{
//cout<<"coef:"<<coef<<endl;
	...
	foreach_lower_priority_task(tasks.get_tasks(), ti, tl)
	{
		...
		lp_fmlp_indirected_blocking(ti, *tl, tasks, vars, exp, coef*(1.0/p_num));

		lp_fmlp_preemption_blocking(ti, *tl, tasks, vars, exp, coef*(1.0/p_num));
	}
}
/*
void lp_fmlp_objective(Task& ti, TaskSet& tasks, ProcessorSet& processors, LinearProgram& lp, FMLPMapper& vars, LinearExpression *obj)
{
	...
	foreach_lower_priority_task(tasks.get_tasks(), ti, tl)
	{
		...
		lp_fmlp_indirected_blocking(ti, *tl, tasks, vars, obj, 1/p_num);

		lp_fmlp_preemption_blocking(ti, *tl, tasks, vars, obj, 1/p_num);
	}

	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		lp_fmlp_directed_blocking(ti, *tx, tasks, vars, obj, 1/p_num);
	}
...
void lp_fmlp_objective(Task& ti, TaskSet& tasks, ProcessorSet& processors, LinearProgram& lp, FMLPMapper& vars, LinearExpression *obj)
{
	...
	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		...
		lp_fmlp_directed_blocking(ti, *tx, tasks, vars, obj, 1);

		lp_fmlp_indirected_blocking(ti, *tx, tasks, vars, obj, 1/p_num);

		lp_fmlp_preemption_blocking(ti, *tx, tasks, vars, obj, 1/p_num);
	}
...
void lp_fmlp_declare_variable_bounds(Task& ti, TaskSet& tasks, LinearProgram& lp, FMLPMapper& vars)
{
	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		uint x = tx->get_id();
...
//cout<<"task:"<<l<<endl;	
		...
		lp_fmlp_indirected_blocking(ti, *tl, tasks, vars, obj, 1.0/p_num);

		lp_fmlp_preemption_blocking(ti, *tl, tasks, vars, obj, 1.0/p_num);
	}

	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		lp_fmlp_directed_blocking(ti, *tx, tasks, vars, obj, 1.0);
	}
...
ulong fmlp_wait_time_bound(Task& ti, uint resource_id, TaskSet& tasks, ProcessorSet& processors, ResourceSet& resources)
{
	...
	foreach_lower_priority_task(tasks.get_tasks(), ti, tl)
	{
		...
		if(tl->is_request_exist(resource_id))
			temp = fmlp_holding_bound(ti, *tl, resource_id, tasks, processors, resources);
	...
	do
	{
		...
		foreach_higher_priority_task(tasks.get_tasks(), ti, th)
		{
			if(th->is_request_exist(resource_id))
			{
				...
				ulong H_h_q = fmlp_holding_bound(ti, *th, resource_id, tasks, processors, resources);
...
{
	lp_fmlp_constraint_1(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_2(ti, tasks, processors, resources, lp, vars);
	lp_fmlp_constraint_3(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_4(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_5(ti, tasks, resources, lp, vars);
vars.seal();
	lp_fmlp_declare_variable_bounds(ti, tasks, lp, vars);
	lp_fmlp_constraint_6(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_7(ti, tasks, processors, resources, lp, vars);
	lp_fmlp_constraint_8(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_9(ti, tasks, resources, lp, vars);
	lp_fmlp_constraint_10(ti, tasks, resources, lp, vars);
}
...
void lp_fmlp_constraint_1(Task& ti, TaskSet& tasks, ResourceSet& resources, LinearProgram& lp, FMLPMapper& vars)
{
	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		...
		lp_fmlp_directed_blocking(ti, *tx, tasks, vars, exp);

		lp_fmlp_indirected_blocking(ti, *tx, tasks, vars, exp);

		lp_fmlp_preemption_blocking(ti, *tx, tasks, vars, exp);
...
{	
	foreach_task_except(tasks.get_tasks(), ti, tx)
	{
		...
		lp_fmlp_OD(ti, tasks, processors, vars, exp, -1);
