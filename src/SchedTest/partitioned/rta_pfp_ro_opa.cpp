...
RTA_PFP_RO_OPA::RTA_PFP_RO_OPA():PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented") {}

RTA_PFP_RO_OPA::RTA_PFP_RO_OPA(TaskSet tasks, ProcessorSet processors, ResourceSet resources):PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented")
{
	this->tasks = tasks;
...
ulong RTA_PFP_RO_OPA::blocking_bound(Task& ti, uint r_id)
{
	ulong bound = 0;
...
ulong RTA_PFP_RO_OPA::request_bound(Task& ti, uint r_id)
{
...
ulong RTA_PFP_RO_OPA::formula_30(Task& ti, uint p_id, ulong interval)
{
	ulong miu = 0;
...
ulong RTA_PFP_RO_OPA::angent_exec_bound(Task& ti, uint p_id, ulong interval)
{
	ulong deadline = ti.get_deadline();
	ulong lambda = 0;
	foreach(ti.get_requests(), request)
...
ulong RTA_PFP_RO_OPA::NCS_workload(Task& ti, ulong interval)
{
...
ulong RTA_PFP_RO_OPA::CS_workload(Task& ti, uint resource_id, ulong interval)
{
	ulong p = ti.get_period();
	ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_OPA::response_time_UA(Task& ti)
{
...
ulong RTA_PFP_RO_OPA::response_time_AP(Task& ti)
{
...
ulong RTA_PFP_RO_OPA::response_time_SP(Task& ti)
{
	uint p_id = ti.get_partition();
	ulong test_bound = ti.get_deadline();
	ulong test_start = ti.get_wcet_non_critical_sections();
...
bool RTA_PFP_RO_OPA::alloc_schedulable()
{
	return true;
}
