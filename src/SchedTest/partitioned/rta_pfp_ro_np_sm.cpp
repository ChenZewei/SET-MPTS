...
RTA_PFP_RO_NP_SM::RTA_PFP_RO_NP_SM():PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented") {}

RTA_PFP_RO_NP_SM::RTA_PFP_RO_NP_SM(TaskSet tasks, ProcessorSet processors, ResourceSet resources):PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented")
{
	this->tasks = tasks;
...
ulong RTA_PFP_RO_NP_SM::blocking_bound(Task& ti, uint r_id)
{
	ulong bound = 0;
...
ulong RTA_PFP_RO_NP_SM::request_bound(Task& ti, uint r_id)
{
...
ulong RTA_PFP_RO_NP_SM::formula_30(Task& ti, uint p_id, ulong interval)
{
	ulong miu = 0;
...
ulong RTA_PFP_RO_NP_SM::angent_exec_bound(Task& ti, uint p_id, ulong interval)
{
	ulong deadline = ti.get_deadline();
	ulong lambda = 0;
	foreach(ti.get_requests(), request)
...
ulong RTA_PFP_RO_NP_SM::NCS_workload(Task& ti, ulong interval)
{
...
ulong RTA_PFP_RO_NP_SM::CS_workload(Task& ti, uint resource_id, ulong interval)
{
	ulong p = ti.get_period();
	ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_NP_SM::response_time_AP(Task& ti)
{
...
ulong RTA_PFP_RO_NP_SM::response_time_SP(Task& ti)
{
	uint p_id = ti.get_partition();
	ulong test_bound = ti.get_deadline();
	ulong test_start = ti.get_wcet_non_critical_sections();
...
bool RTA_PFP_RO_NP_SM::is_first_fit_for_tasks_schedulable(uint start_processor)
{
	bool schedulable;
