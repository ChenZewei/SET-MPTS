...
RTA_PFP_RO_FEASIBLE::RTA_PFP_RO_FEASIBLE():PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented") {}

RTA_PFP_RO_FEASIBLE::RTA_PFP_RO_FEASIBLE(TaskSet tasks, ProcessorSet processors, ResourceSet resources):PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented")
{
	this->tasks = tasks;
...
bool RTA_PFP_RO_FEASIBLE::condition_3()
{
	foreach(tasks.get_tasks(), ti)
	{
		foreach(ti->get_requests(), request_q)
		{
			...
			{
				if(tl->get_deadline() <= ti->get_deadline() || !(tl->is_request_exist(q)))
					continue;
...
ulong RTA_PFP_RO_FEASIBLE::DBF_R(Task& ti, uint r_id, ulong interval)
{
	ulong deadline = ti.get_deadline();
	ulong period = ti.get_period();
	Request& request = ti.get_request_by_id(r_id);
