...
RTA_PFP_RO_NP::RTA_PFP_RO_NP():PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented") {}

RTA_PFP_RO_NP::RTA_PFP_RO_NP(TaskSet tasks, ProcessorSet processors, ResourceSet resources):PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "", "Resource-Oriented")
{
	this->tasks = tasks;
...
ulong RTA_PFP_RO_NP::blocking_bound(Task& ti, uint r_id)
{
	ulong bound = 0;
...
ulong RTA_PFP_RO_NP::request_bound(Task& ti, uint r_id)
{
...
ulong RTA_PFP_RO_NP::angent_exec_bound(Task& ti, uint p_id)
{
	ulong deadline = ti.get_deadline();
	ulong lambda = 0;
...
ulong RTA_PFP_RO_NP::NCS_workload(Task& ti, ulong interval)
{
...
ulong RTA_PFP_RO_NP::CS_workload(Task& ti, uint resource_id, ulong interval)
{
	ulong p = ti.get_period();
	ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_NP::response_time_AP(Task& ti)
{
...
ulong RTA_PFP_RO_NP::response_time_SP(Task& ti)
{
	uint p_id = ti.get_partition();
	ulong test_bound = ti.get_deadline();
	ulong test_start = ti.get_wcet_non_critical_sections();
