...
ILP_RTA_PFP_spinlock::ILP_RTA_PFP_spinlock(): PartitionedSched(true, RTA, FIX_PRIORITY, SPIN, "", "spinlock") {}

ILP_RTA_PFP_spinlock::ILP_RTA_PFP_spinlock(TaskSet tasks, ProcessorSet processors, ResourceSet resources): PartitionedSched(true, RTA, FIX_PRIORITY, SPIN, "", "spinlock")
{
	this->tasks = tasks;
...
cout<<"/===========Task "<<i<<"===========/"<<endl;
			...
			double result;
			ILP_SpinLock_construct_exp(vars, exp, ILPSpinLockMapper::RESPONSE_TIME, i);
			result = rb_solution->evaluate(*exp);
cout<<"response time of task "<<i<<":"<<result<<endl;

			ILP_SpinLock_construct_exp(vars, exp, ILPSpinLockMapper::INTERFERENCE_TIME, i);
			result = rb_solution->evaluate(*exp);
cout<<"interference time of task "<<i<<":"<<result<<endl;

			ILP_SpinLock_construct_exp(vars, exp, ILPSpinLockMapper::BLOCKING_TIME, i);
			result = rb_solution->evaluate(*exp);
...
cout<<"|===========Locailty Assignment===========|"<<endl;
			...
			{
				ILP_SpinLock_construct_exp(vars, exp, ILPSpinLockMapper::LOCALITY_ASSIGNMENT, i, k);
				result = rb_solution->evaluate(*exp);
...
cout<<"|===========Priority Assignment===========|"<<endl;
			...
			{
				ILP_SpinLock_construct_exp(vars, exp, ILPSpinLockMapper::PRIORITY_ASSIGNMENT, i, p);
				result = rb_solution->evaluate(*exp);
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_construct_exp(ILPSpinLockMapper& vars, LinearExpression *exp, uint type, uint part_1, uint part_2, uint part_3, uint part_4)
{
	exp->get_terms().clear();
	uint var_id;
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_set_objective(LinearProgram& lp, ILPSpinLockMapper& vars)
{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_add_constraints(LinearProgram& lp, ILPSpinLockMapper& vars)
{
//cout<<"Set C1"<<endl;
...
////////////////////Expressions////////////////////
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_1(LinearProgram& lp, ILPSpinLockMapper& vars)
{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_2(LinearProgram& lp, ILPSpinLockMapper& vars)
{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_2_1(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint t_num = tasks.get_tasks().size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_3(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_4(LinearProgram& lp, ILPSpinLockMapper& vars)
{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_5(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	foreach(tasks.get_tasks(), ti)
	{
		uint i = ti->get_index();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_6(LinearProgram& lp, ILPSpinLockMapper& vars)
{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_7(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	foreach(tasks.get_tasks(), task)
	{
		uint i = task->get_index();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_8(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	foreach(tasks.get_tasks(), ti)
	{
		uint i = ti->get_index();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_9(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	foreach(tasks.get_tasks(), ti)
	{
		uint i = ti->get_index();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_10(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	foreach(tasks.get_tasks(), ti)
	{
		uint i = ti->get_index();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_11(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
	foreach(tasks.get_tasks(), ti)
	{
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_12(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_13(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint t_num = tasks.get_taskset_size();
	uint p_num = processors.get_processor_num();
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_14(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_15(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_16(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_17(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_18(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
	uint r_num = resources.get_resourceset_size();
...
void ILP_RTA_PFP_spinlock::ILP_SpinLock_constraint_19(LinearProgram& lp, ILPSpinLockMapper& vars)
{
	uint p_num = processors.get_processor_num();
	uint r_num = resources.get_resourceset_size();
