...
ulong gn_gpf_workload_nc(Task& task, long interval)
{
...
ulong gn_gpf_workload_ci(Task& task, long interval)
{
	long C_i = task.get_wcet();
	long T_i = task.get_period();
...
cout<<"dddd"<<endl;
cout<<(T_i - R_i)<<endl;
cout<<(interval - C_i)<<endl;
cout<<max(zero, ((interval - C_i)))%T_i<<endl;
cout<<max(zero, ((interval - C_i)))%T_i - (T_i - R_i)<<endl;
cout<<max(zero, max(zero, ((interval - C_i)))%T_i - (T_i - R_i));
cout<<a<<endl;
*/
	return floor(max(zero, interval - C_i)/T_i)*C_i + C_i + a;
}

ulong gn_gpf_interference_nc(Task& task_k, Task& task_i, ulong interval)
{
	return min(interval - task_k.get_wcet() + 1, max((ulong)0, gn_gpf_workload_nc(task_i,interval)));
}

ulong gn_gpf_interference_ci(Task& task_k, Task& task_i, ulong interval)
{
	return min(interval - task_k.get_wcet() + 1, max((ulong)0, gn_gpf_workload_ci(task_i,interval)));
}

ulong gn_gpf_omega(TaskSet& tasks, uint t_id, ProcessorSet& processors, ulong interval)
{
	...
	for(uint i = 0; i < t_id; i++)
	{
		...
		Carry_in temp;
		temp.id = i;
		temp.gap = gn_gpf_interference_ci(task_k, task_i, interval) - gn_gpf_interference_nc(task_k, task_i, interval);
		carry_in.push_back(temp);
	}
//cout<<"1111"<<endl;
	for(uint i = 0; i < carry_in.size(); i++)
	{
//		cout<<"id:"<<carry_in[i].id<<" "<<"gap:"<<carry_in[i].gap<<endl;
	}
...
//cout<<ci_num<<endl;
//cout<<"3333"<<endl;
//cout<<tasks.get_taskset_size()<<endl;
//cout<<carry_in.size()<<endl;
//cout<<ci_num<<endl;
//tasks.display();
...
//cout<<"4444"<<endl;
	for(uint i = 0; i < t_id; i++)
	{
		...
		{
			I_ci_sum += gn_gpf_interference_ci(task_k, task_i, interval);
//			cout<<"carry_in id:"<<task_i.get_id()<<endl;
		}		
		else
			I_nc_sum += gn_gpf_interference_nc(task_k, task_i, interval);
	}
	for(uint i = 0; i < ci_num; i++)
	{
		uint id = carry_in[i].id;
...
ulong gn_gpf_rta(TaskSet& tasks, ProcessorSet& processors, uint t_id)
{
	Task& task = tasks.get_task_by_id(t_id);
	ulong wcet = task.get_wcet();
	ulong test_end = task.get_deadline();
...
//tasks.display();
	while(response < test_end)
	{
		omega = gn_gpf_omega(tasks, t_id, processors, response);
...
bool is_gpf_rta_schedulable(TaskSet& tasks, ProcessorSet& processors, uint TEST_TYPE)
{
		...
		for (uint t_id = 0; t_id < tasks.get_taskset_size(); t_id ++)
		{
			...
			switch (TEST_TYPE)
			{
				...
				case 1://bc-rta
					...
					break;
				case 2://gn-rta
					response_bound = gn_gpf_rta(tasks, processors, t_id);
					break;
				default://native-rta
					response_bound = native_gfp_rta(tasks, processors, t_id);
					break;
